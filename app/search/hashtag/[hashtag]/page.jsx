...
const page = ({params}) => {
  ...
  return (
    <div className='py-12'>
         <h1 className='text-[24px] uppercase font-black text-center mb-8'>Search for {params.hashtag} by HashTag</h1>
      <div className='gap-16 flex flex-col-reverse justify-center items-center'>
        {feedData?.length > 0 ? feedData?.filter((item) => item.tag === `#${params.hashtag}`  || item.tag.includes(params.hashtag)).map((post, index) => {
            return(
        ...
        <div>
            <p>No search for {params.hashtag}</p>
        </div>}
      </div>
    </div>
  )
