  ...
  return (
    <div className='w-full py-24'>
        ...
        <div className='flex justify-center items-center flex-col gap-5 mt-8'>
            {userData?.posts?.length > 0 ? userData?.followers?.map((follower, i) => {
                return(
                    <div key={i} className='flex justify-between p-2 px-3 rounded-full items-center border-2 w-[300px]'>
                        <Link href={`/profile/${follower.id}`} className='flex justify-center gap-4 items-center'>
                            <img className='rounded-full w-12' src={follower?.image}  />
