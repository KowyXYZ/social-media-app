  ...
  return (
    <div className='py-12'>
      <div className='gap-16 flex flex-col-reverse justify-center items-center'>
        {feedData?.length > 0 ? feedData?.filter((item) => item.text.toLowerCase() === params.searchText  || item.text.includes(params.searchText)).map((post, index) => {
            return(
        ...
        <div>
            <p>No search for {params.searchText}</p>
        </div>}
      </div>
    </div>
  )
